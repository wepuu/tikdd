...
export function GrowthReadiness({ content, publication, seo, settings }: Props) {
  ...
  return <section className="growth-readiness" id="growth">
    ...
    {model.blockers.length ? <div className="growth-blockers"><WarningCircle size={16} /><span>待处理：{model.blockers.join(" · ")}</span><a href="#publishing">查看发布准备 <ArrowUpRight size={13} /></a></div> : <div className="growth-clear"><CheckCircle size={16} weight="fill" /><span>当前没有增长准备阻塞；平台按生产路由与内容完整度决定索引资格。</span></div>}
  </section>;
}
