...
export class AdminContentManagementService {
  ...
  async publish(raw:unknown,actor:string){
    ...
    const pages=view.pages.filter(page=>page.state==="ready"||page.state==="published").map(page=>({pageId:page.pageId,locale:page.locale,pageType:page.pageType,platform:page.platform,content:page.content,seo:page.seo}));
     const defaultShared=view.sharedContent.find(item=>item.locale===locales.find(item=>item.isDefault)?.locale);
     const siteIntegrations=defaultShared?.content.siteIntegrations??{googleAnalyticsMeasurementId:null,googleAdsensePublisherId:null};
     const sharedContent=view.sharedContent.filter(item=>item.state==="ready"||item.state==="published").map(item=>{const {siteIntegrations:_siteIntegrations,...localizedContent}=item.content;return{locale:item.locale,...localizedContent};});const content={locales,pages,sharedContent,siteIntegrations};const contentHash=createHash("sha256").update(JSON.stringify(content)).digest("hex");const snapshotId=`snap_${createHash("md5").update(`${command.deployment}:${next}:${contentHash}`).digest("hex")}`;
  ...
  async getSettingsRecoveryView(){
    ...
    const siteIdentity=content.locales.map(({locale,effective})=>{const item=identityByLocale.get(locale);return{locale,revision:item?.revision??null,state:item?.state??"missing" as const,siteName:item?.content.siteName??"TikDD",navigationLabel:item?.content.navigationLabel??effective.displayName,footerTagline:item?.content.footerTagline??"TikDD",legalNoticeMarkdown:item?.content.legalNoticeMarkdown??"Publish only reviewed public content.",defaultSocial:{title:item?.content.defaultSocialTitle??null,description:item?.content.defaultSocialDescription??null,imageAssetId:item?.content.defaultSocialImageAssetId??null}};});
     ...
     const defaultShared=identityByLocale.get(defaultLocale);
     const siteIntegrations=defaultShared?.content.siteIntegrations??{googleAnalyticsMeasurementId:null,googleAdsensePublisherId:null};
     const publishedSiteIntegrations=active
       ? PublishedContentSnapshotSchema.parse(active.payload).siteIntegrations
       : {googleAnalyticsMeasurementId:null,googleAdsensePublisherId:null};
