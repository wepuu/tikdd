...
export function verifyWorkItem16Static() {
  ...
  assert(/^  calibration-data:\r?\n    internal: true\r?\n    ipam:/m.test(compose), "The internal calibration network must declare stable IPAM.");
  ...
  for (const binding of ["TIKDD_DATA_SUBNET", "TIKDD_DATA_GATEWAY", "TIKDD_CALIBRATION_DATA_SUBNET", "TIKDD_CALIBRATION_DATA_GATEWAY", "TIKDD_HOST_INGRESS_SUBNET", "TIKDD_HOST_INGRESS_GATEWAY", "TIKDD_PROVIDER_EGRESS_SUBNET", "TIKDD_PROVIDER_EGRESS_GATEWAY"]) {
  ...
  assert(/^TIKDD_CALIBRATION_DATA_SUBNET=172\.30\.43\.0\/24$/m.test(productionEnvironment), "The reviewed NL calibration subnet changed unexpectedly.");
  ...
  const expectedNetworks = {
    postgres: ["data", "calibration-data"], redis: ["data", "calibration-data"], web: ["data", "host-ingress"], api: ["data", "host-ingress"],
  ...
  return { serviceCount: allServices.length, networkCount: 4, publishedServiceCount: 4 };
}
