...
if (!process.env.DATABASE_URL || !process.env.REDIS_URL) {
  throw new Error("DATABASE_URL and REDIS_URL are required for the work item 11 baseline gate.");
}
...
const stages = [
  ...
  ["evidence-contracts-and-privacy", ["test:work-item-11"]],
  ["utc-replay-and-restrictive-evaluator", ["verify:evidence"]],
  ...
  ["pending-real-time-evidence-contract", ["verify:work-item-10:evidence"]],
...
for (const [stage, args] of stages) {
  ...
  process.stdout.write(`${JSON.stringify({ event: "work_item_11_baseline_stage_start", stage })}\n`);
  ...
  process.stdout.write(`${JSON.stringify({
    event: "work_item_11_baseline_stage_complete",
    stage,
    passed,
...
process.stdout.write(`${JSON.stringify({
  event: "work_item_11_baseline_verification_complete",
  passed: true,
  liveProviderNetwork: false,
  pilotTrafficEnabled: false,
  operationalObservationComplete: false,
  publicOpenApiChanged: false,
