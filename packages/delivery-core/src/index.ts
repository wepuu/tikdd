...
export const SNAPTIK_MONSTER_TIKTOK_MEDIA_HOST_POLICY = DeliveryHostPolicySchema.parse({
  id: "snaptik-monster-tiktok-media-v1",
...
export const PINTEREST_VIDEODOWNLOADER_PINTEREST_MEDIA_HOST_POLICY = DeliveryHostPolicySchema.parse({
  id: "pinterest-videodownloader-pinterest-media-v1",
  providerId: "pinterest-videodownloader",
  modes: ["redirect"],
  hosts: ["v1.pinimg.com"]
});

export const VIDDOWN_NET_VIMEO_MEDIA_HOST_POLICY = DeliveryHostPolicySchema.parse({
  id: "viddown-net-vimeo-media-v1",
...
export const NINE_X_BUDDY_XHAMSTER_ARTIFACT_HOST_POLICY = DeliveryHostPolicySchema.parse({
  id: "9xbuddy-xhamster-artifact-v1",
  providerId: "9xbuddy",
  modes: ["redirect"],
  hosts: ["ab.9xbud.com"],
  browserHandoff: "navigate"
...
export function assertDeliveryTargetPolicy(input: {
  ...
  if (url.port) {
    throw new Error("The delivery target must use the standard HTTPS port.");
  }
